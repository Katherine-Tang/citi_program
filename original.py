...
# Load data
fx_daily = pd.read_excel('./Desktop/FX/usd_cnh_2015.xlsx', parse_dates=["Date"])

fx_daily = risk_score_definitions(fx_daily)
us_cpu = pd.read_excel('./Desktop/FX/features/subEPU/US-CPU_2015.xlsx')
window_size = 10

# FX calculations
# fx_daily[f"FX_Volatility_roll{window_size}"] = fx_daily["FX_Daily_Returns"].rolling(window=window_size).std()
daily_folders = ['./Desktop/FX/features/GPR', './Desktop/FX/features/other_assets', './Desktop/FX/features/sentiment_score', './Desktop/FX/features/IR']
daily_index = pd.read_excel('./Desktop/FX/index_daily.xlsx')
merged_df_daily = merge_selected_folders_with_fx(daily_index, daily_folders, './Desktop/FX/features/merged/merged_daily_data.xlsx')
...
monthly_folders = ['./Desktop/FX/features/CPI', './Desktop/FX/features/EPU', "./Desktop/FX/features/subEPU", "./Desktop/FX/features/BoP", './Desktop/FX/features/employment', './Desktop/FX/features/FER', './Desktop/FX/features/currency_supply']
merged_df_month = merge_selected_folders_with_fx(us_cpu, monthly_folders, './Desktop/FX/features/merged/merged_monthly_data.xlsx')
...
qr_folders = ['./Desktop/FX/features/GDP']
qr_index = pd.read_excel('./Desktop/FX/qr_index.xlsx')
merged_df_qr = merge_selected_folders_with_fx(qr_index, qr_folders, './Desktop/FX/features/merged/merged_qr_data.xlsx')
...
plot_real_pred_reg(y_test_reg, y_pred_reg,'/Users/kittentang/Desktop/results/xg_volatility_pred_real.png')

threshold_type  = 'value'
visualize_detection_class(y_test_reg, y_pred_reg,y_test_class, y_pred_class,threshold_type,True,True, 0.003,0.95,f'/Users/kittentang/Desktop/results/xg_classify_{threshold_type}.png',
                          f'/Users/kittentang/Desktop/results/xg_classify_CI.png', f'/Users/kittentang/Desktop/results/xg_classify_class2.png')
...
def backtest_risk_signals(pair_data, risk_predictions, strategy_type='enhanced', 
    ...
    # 计算收益回撤比
    ret_dd_ratio = annual_return / max_drawdown if max_drawdown > 0 else float('inf')
...
if __name__ == "__main__":
    ...
    # 添加多货币对处理
    multi_pairs_file = os.path.expanduser('~/Desktop/citi program/all_currency.xlsx')
    ...
    visualize_risk_distribution(risk_distribution, weighted_risk, 
                               os.path.expanduser('~/Desktop/results/risk_distribution.png'))
...
# 添加参数优化和回测配置函数
def optimize_backtest_parameters(pair_data, risk_predictions, param_sets=None, max_drawdown_limit=0.15):
    """
    根据最大回撤限制和其他参数微调回测参数，寻找最优参数配置
    
    参数:
    pair_data: 包含价格数据的DataFrame
    risk_predictions: 风险类别预测结果
    param_sets: 自定义参数集合列表，每个元素为包含所有参数的字典
    max_drawdown_limit: 最大可接受回撤比例
    
    返回:
    best_params: 最佳参数组合
    best_results: 使用最佳参数的回测结果
    """
    if isinstance(risk_predictions, np.ndarray):
        risk_predictions = pd.Series(risk_predictions, index=pair_data.index)
    ...
    if param_sets is None:
        param_sets = [
            # 保守策略 - 低风险，短期交易，严格止损
            {
                ...
                'volatility_multiplier': 0.8,  # 波动率乘数更小
                'trend_confirmation_days': 5,  # 需要更多天确认趋势
                'rsi_buy_threshold': 40,  # RSI低于40才买入
                'rsi_sell_threshold': 65   # RSI高于65才卖出
            },
            # 中等风险策略 - 平衡型
            {
                'strategy_type': 'enhanced',
                ...
                'max_holding_days': 15,   # 最大持仓15天
                'volatility_multiplier': 1.0,  # 标准波动率乘数
                'trend_confirmation_days': 3,  # 3天确认趋势
                'rsi_buy_threshold': 35,  # RSI低于35才买入
                'rsi_sell_threshold': 65   # RSI高于65才卖出
            },
            # 积极策略 - 更多交易，更长持仓
            {
                ...
                'volatility_multiplier': 1.2,  # 波动率乘数更大
                'trend_confirmation_days': 2,  # 只需2天确认趋势
                'rsi_buy_threshold': 30,  # RSI低于30才买入
                'rsi_sell_threshold': 70   # RSI高于70才卖出
            },
            # 回调优化策略 - 关注回撤控制
            {
                'strategy_type': 'combined',
                'risk_threshold': 2,
                'stop_loss_pct': 0.018,  # 1.8%止损
                'take_profit_pct': 0.036, # 3.6%止盈
                'max_holding_days': 12,   # 最大持仓12天
                'volatility_multiplier': 0.9,  # 波动率乘数较小
                'trend_confirmation_days': 4,  # 4天确认趋势
                'rsi_buy_threshold': 38,  # RSI低于38才买入
                'rsi_sell_threshold': 62   # RSI高于62才卖出
            },
            # 长期趋势策略 - 长期持仓
            {
                'strategy_type': 'trend_filter',
                'risk_threshold': 2,
                'stop_loss_pct': 0.022,  # 2.2%止损
                'take_profit_pct': 0.055, # 5.5%止盈
                'max_holding_days': 25,   # 最大持仓25天
                'volatility_multiplier': 1.1,  # 波动率乘数略高
                'trend_confirmation_days': 5,  # 5天确认趋势
                'rsi_buy_threshold': 35,  # RSI低于35才买入
                'rsi_sell_threshold': 68   # RSI高于68才卖出
            }
        ]
    ...
    for params in param_sets:
        ...
        # 修改回测函数调用，传入更多参数
        backtest_result = custom_backtest_risk_signals(
            pair_data=pair_data,
        ...
        # 过滤掉回撤超过限制的策略
        if backtest_result['max_drawdown'] > max_drawdown_limit * 100:
            print(f"回撤过大 ({backtest_result['max_drawdown']:.2f}%)，跳过此组参数")
            continue
        
        # 计算综合评分 (50% 夏普比率, 30% 年化收益率, 20% 胜率)
        sharpe_score = backtest_result['sharpe_ratio'] * 50 / 3  # 标准化到约50分
        return_score = backtest_result['annual_return'] * 30 / 20  # 标准化到约30分
        win_rate_score = backtest_result['win_rate'] * 20 / 100  # 标准化到20分
        
        # 回撤惩罚因子 (回撤越大，得分越低)
        drawdown_penalty = 1 - (backtest_result['max_drawdown'] / (max_drawdown_limit * 100))
        drawdown_penalty = max(0.5, drawdown_penalty)  # 至少保留50%的分数
        
        # 计算综合得分
        total_score = (sharpe_score + return_score + win_rate_score) * drawdown_penalty
        result_entry['score'] = total_score
        
        results.append(result_entry)
        
        print(f"评分: {total_score:.2f}, 夏普: {backtest_result['sharpe_ratio']:.2f}, "
              f"年收益: {backtest_result['annual_return']:.2f}%, 胜率: {backtest_result['win_rate']:.2f}%")
    
    # 根据评分排序
    results.sort(key=lambda x: x['score'], reverse=True)
    
    if not results:
        print("没有找到符合回撤限制的参数组合")
        return None, None
    ...
    return best['params'], best['results']

def custom_backtest_risk_signals(pair_data, risk_predictions, strategy_type='enhanced', 
                               risk_threshold=2, stop_loss_pct=0.02, take_profit_pct=0.04,
                               max_holding_days=20, volatility_multiplier=1.0,
                               trend_confirmation_days=3, rsi_buy_threshold=35, rsi_sell_threshold=65):
    """
    具有更多可自定义参数的风险信号回测函数
    ...
    volatility_multiplier: 波动率计算的乘数
    trend_confirmation_days: 趋势确认所需天数
    ...
    # 趋势确认 - 设置连续N天趋势一致的条件
    backtest_df['sma_diff'] = backtest_df['sma_20'] - backtest_df['sma_50']
    backtest_df['trend_direction'] = np.sign(backtest_df['sma_diff'])
    backtest_df['trend_confirmed'] = False
    
    # 需要确认连续几天趋势一致
    for i in range(trend_confirmation_days, len(backtest_df)):
        consistent_trend = True
        current_trend = backtest_df['trend_direction'].iloc[i]
        for j in range(1, trend_confirmation_days):
            if backtest_df['trend_direction'].iloc[i-j] != current_trend:
                consistent_trend = False
                break
        backtest_df.loc[backtest_df.index[i], 'trend_confirmed'] = consistent_trend
    
    backtest_df['trend'] = np.where(backtest_df['sma_20'] > backtest_df['sma_50'], 1, -1)
    
    # 计算波动率用于动态止损/止盈
    backtest_df['volatility'] = backtest_df['close'].pct_change().rolling(window=20).std() * np.sqrt(20) * volatility_multiplier
    ...
    for i in range(1, len(backtest_df)):
        ...
        # 动态止损/止盈基于波动率
        current_volatility = backtest_df['volatility'].iloc[i]
        dynamic_stop_loss = max(stop_loss_pct, current_volatility * 1.0)
        dynamic_take_profit = max(take_profit_pct, current_volatility * 2.0)
        ...
        trend = backtest_df['trend'].iloc[i]
        rsi = backtest_df['rsi'].iloc[i]
        trend_confirmed = backtest_df['trend_confirmed'].iloc[i]
        ...
        if strategy_type == 'basic':
            ...
            # 趋势过滤策略：只在趋势方向一致且已确认时交易
            if position == 0 and trend_confirmed:  # 没有持仓时且趋势已确认
                if risk_class == 0 and trend == 1 and rsi < rsi_buy_threshold:  # 低风险信号 + 上升趋势 + RSI低于阈值，做多
                    position = 1
                    entry_price = curr_price
                    entry_date = date
                    trades.append({
                ...
                elif risk_class == risk_threshold and trend == -1 and rsi > rsi_sell_threshold:  # 高风险信号 + 下降趋势 + RSI高于阈值，做空
                    position = -1
                    entry_price = curr_price
                    entry_date = date
                    trades.append({
                ...
                # 平仓条件：高风险信号或趋势反转或RSI超买
                if risk_class == risk_threshold or (trend == -1 and trend_confirmed) or rsi > rsi_sell_threshold:
                    trades[-1].update({
                        'exit_date': date,
                        'exit_price': curr_price,
                        'pnl_pct': (curr_price - entry_price) / entry_price,
                ...
                # 平仓条件：低风险信号或趋势反转或RSI超卖
                if risk_class == 0 or (trend == 1 and trend_confirmed) or rsi < rsi_buy_threshold:
                    trades[-1].update({
                        'exit_date': date,
                        'exit_price': curr_price,
                        'pnl_pct': (entry_price - curr_price) / entry_price,
        ...
        elif strategy_type == 'enhanced' or strategy_type == 'combined':
            # 增强策略：综合风险信号、趋势、RSI和价格关系
            if position == 0:  # 没有持仓时
                # 买入条件：低风险 + 上升趋势 + RSI不超买 + 价格关系确认
                if (risk_class == 0 and trend == 1 and rsi < rsi_buy_threshold and 
                    (strategy_type != 'combined' or trend_confirmed) and
                    curr_price > backtest_df['sma_20'].iloc[i]):
                    position = 1
                    entry_price = curr_price
                    entry_date = date
                ...
                # 卖出条件：高风险 + 下降趋势 + RSI不超卖 + 价格关系确认
                elif (risk_class == risk_threshold and trend == -1 and rsi > rsi_sell_threshold and
                      (strategy_type != 'combined' or trend_confirmed) and
                      curr_price < backtest_df['sma_20'].iloc[i]):
                    position = -1
                    entry_price = curr_price
            ...
            # 多头平仓条件
                ...
                if ((risk_class == risk_threshold and (strategy_type == 'combined' or trend == -1)) or 
                    rsi > rsi_sell_threshold + 5 or  # 额外5点缓冲
                    (trend == -1 and trend_confirmed and strategy_type == 'combined')):
                    trades[-1].update({
                        'exit_date': date,
                        'exit_price': curr_price,
                        'pnl_pct': (curr_price - entry_price) / entry_price,
            ...
            # 空头平仓条件
                ...
                if ((risk_class == 0 and (strategy_type == 'combined' or trend == 1)) or 
                    rsi < rsi_buy_threshold - 5 or  # 额外5点缓冲
                    (trend == 1 and trend_confirmed and strategy_type == 'combined')):
                    trades[-1].update({
                        'exit_date': date,
                        'exit_price': curr_price,
...
def process_multi_currency_pairs(file_path):
    ...
    for pair in currency_pairs:
        ...
        # 定义自定义参数集 - 可以根据不同货币对特性调整
        if pair == 'USDCNH':
            # 对USDCNH使用更严格的最大回撤控制
            max_drawdown_limit = 0.12  # 12%最大回撤限制
            custom_params = [
                # USDCNH专用参数组合1 - 保守策略
                {
                    'strategy_type': 'enhanced', 
                    ...
                    'trend_confirmation_days': 4,
                    'rsi_buy_threshold': 38,
                    'rsi_sell_threshold': 65
                },
                # USDCNH专用参数组合2 - 针对较高波动性
                {
                    'strategy_type': 'trend_filter', 
                    'risk_threshold': 2,
                    'stop_loss_pct': 0.018,
            ...
            custom_params = None  # 使用默认参数集
        
        # 运行参数优化
        best_params, best_results = optimize_backtest_parameters(
            pair_data=test_data,
            risk_predictions=y_pred_class_series,
        ...
        # 保存最优参数回测结果
        if best_results:
            results[pair]['backtest_results'] = best_results
            results[pair]['best_params'] = best_params
            ...
            print(f"{pair}参数优化完成！")
            print(f"最佳策略: {best_params['strategy_type']}")
            print(f"总交易次数: {best_results['total_trades']}")
            print(f"胜率: {best_results['win_rate']:.2f}%")
            print(f"平均收益: {best_results['avg_return']:.2f}%")
            print(f"年化收益: {best_results['annual_return']:.2f}%")
            print(f"最大回撤: {best_results['max_drawdown']:.2f}%")
            print(f"夏普比率: {best_results['sharpe_ratio']:.2f}")
            print(f"收益回撤比: {best_results['ret_dd_ratio']:.2f}")
        else:
            print(f"未找到符合回撤要求的参数组合，使用基本回测")
            # 使用基本回测作为备选
            backtest_results = backtest_risk_signals(
                test_data, 
                y_pred_class_series,
...
if __name__ == "__main__":
    ...
    if 'USDCNH' in results:
        ...
        # 详细的优化参数
        print("\nUSDCNH最优参数配置:")
        if 'best_params' in usdcnh_results:
            for k, v in usdcnh_results['best_params'].items():
                print(f"{k}: {v}")
        ...
        print(f"最大回撤: {usdcnh_backtest['max_drawdown']:.2f}%")
        print(f"夏普比率: {usdcnh_backtest['sharpe_ratio']:.2f}")
        if 'ret_dd_ratio' in usdcnh_backtest:
            print(f"收益回撤比: {usdcnh_backtest['ret_dd_ratio']:.2f}")
